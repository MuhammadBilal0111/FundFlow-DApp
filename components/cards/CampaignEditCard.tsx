...
import { Edit2, Calendar, DollarSign, Users, ArrowRight, Upload, Wallet, Loader2 } from "lucide-react"
...
export default function CampaignEditCard({
  ...
  project: Project
  handleUpdateProject: (id: number, field: keyof Project, value: string | number) => void
          ...
          <div className="grid gap-4 py-4">
            {field === "imageURL" ? (
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                {field === "description" ? (
                  <Textarea id={field} defaultValue={String(project[field])} className="col-span-8" rows={6} />
                ) : (
                  <Input
                    id={field}
                    type={field === "cost" ? "number" : field === "expiresAt" ? "date" : "text"}
  ...
  const StatusBadge = ({ status }: { status: number }) => {
    const getStatusProps = () => {
      switch (status) {
        case 0:
          return { variant: "outline", className: "bg-green-100 text-green-800 border-green-300", label: "Open" }
        case 1:
          return { variant: "outline", className: "bg-blue-100 text-blue-800 border-blue-300", label: "Approved" }
        case 2:
          return { variant: "outline", className: "bg-red-100 text-red-800 border-red-300", label: "Reverted" }
        case 3:
          return { variant: "outline", className: "bg-gray-100 text-gray-800 border-gray-300", label: "Deleted" }
        case 4:
          return { variant: "outline", className: "bg-gray-100 text-gray-800 border-gray-300", label: "PAIDOUT" }
        default:
          return { variant: "outline", className: "bg-gray-100 text-gray-800 border-gray-300", label: "Unknown" }
      }
    }
    ...
    <Card className="w-full max-w-3xl mb-3 px-1 sm:px-2 md:px-4">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <EditDialog field="title" title="Project Title" description="Edit your project's title" />
        </div>
      </CardHeader>
      <CardContent>
        <div className="mb-6 relative rounded-lg overflow-hidden max-h-[400px]">
          ...
          <div className="absolute top-2 right-2 bg-background/80 rounded-lg p-1">
            <EditDialog field="imageURL" title="Project Image" description="Edit your project's image URL" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-10">
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-4 text-sm text-muted-foreground">
              ...
              <EditDialog field="expiresAt" title="Deadline" description="Edit your project's expiresAt" />
            </div>
          </div>
          <div className="space-y-4">
            <div className="flex flex-col space-y-2">
              <div className="flex items-center justify-between">
                ...
                <EditDialog field="cost" title="Funding Goal" description="Edit your project's funding goal" />
              </div>
              <div className="flex items-center text-2xl font-bold">
                <div className="flex items-center gap-2">
                  <Wallet className="mr-1 h-5 w-5 text-purple-600" />
                  {project?.cost?.toLocaleString()} Eth
                </div>
              </div>
              <div className="h-2 w-full bg-gray-200 rounded-full">
                <motion.div
                  ...
                  style={{
                    width: ((project?.raised / project?.cost) * 100)?.toFixed(0),
                  }}
                  initial={{ width: 0 }}
                  animate={{
                    width: ((project?.raised / project?.cost) * 100)?.toFixed(0),
              ...
              <div className="flex justify-between text-sm text-muted-foreground">
                <span>{((project?.raised / project?.cost) * 100)?.toFixed(0)}% funded</span>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-medium">{project?.backers} backers</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <CardFooter className="flex justify-between sm:flex-row flex-col gap-3 max-sm:w-full">
        <Button variant="outline" onClick={handlePreview} className="hover:bg-secondary max-sm:w-full">
          Preview
        </Button>
